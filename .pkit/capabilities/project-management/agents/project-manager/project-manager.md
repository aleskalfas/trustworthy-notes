...
description: Project-management agent that files, validates, and transitions GitHub issues per the methodology installed by the project-management capability — and additionally takes fuzzy intent + reference material and produces a sliced filing plan behind a single approval gate. Reads the capability's eight schemas at runtime, dispatches on request shape and on the severity vocabulary, and acts via `gh` CLI mutations. Serves PMs and Implementers (per DEC-008); switches mode by which human is directing it.
