...
- **The user has no GitHub account.** The credential cannot come from the user's own identity; it must be provisioned out of band by the maintainer. (The destination repo, being non-secret, is instead supplied as a built-in default — see the Decision.)
...
**Private destination, out-of-band credential.** Feedback with data goes to a **private** feedback repo, never the public code repo. Authentication is a **fine-grained personal access token** scoped to Issues + Contents on that one repo, stored in the user's config and delivered out of band (1Password) by the maintainer — **never compiled into the exe** (`feedback.py`; full design in the scratchpad note `2026-06-17-tn-feedback-design.md`). The repo path may carry a built-in default — a source constant `DEFAULT_FEEDBACK_REPO`, present in dev/sim and the frozen exe alike — that an explicit config value overrides; only the token is never hardcoded. This is sound because **the repo name is not a secret**: anyone may know it, and the repo is private, so the name is useless without the token. Defaulting it removes a setup step for the non-technical user and the maintainer. The accepted tradeoff is that the public source now names the private feedback repo (harmless — the repo is inaccessible without the token), and a third-party adopter inherits this default until they override it in config. Because GitHub has no programmatic issue-attachment endpoint, the bundle is committed into the repo (Contents API, namespaced and timestamped to avoid collisions) and linked from the issue.
...
- **A baked credential is the one thing that breaks this model.** The credential here means specifically the **token**, not the repo: defaulting the non-secret repo name is fine (it is useless without the token), but because the exe is public, any future "convenience" that embeds the token — even scoped, even obfuscated — publishes it. The token is the bright line of this ADR; it is never defaulted, never baked into the exe.
