"""macOS-only launch *mechanics* for the "Send Feedback" desktop droplet (issue #69).
...
double-clicks it) and it opens Terminal running ``tnotes feedback`` against that file.
...
* :func:`create_feedback_droplet` — compile a "Send Feedback" droplet onto the
  user's Desktop via ``osacompile`` (ADR-005), a no-op off macOS.
...
def _applescript_source(tnotes_abs: str) -> str:
    """The droplet's AppleScript: ``on open`` (drag) + ``on run`` (double-click).

    Per dropped file, opens Terminal running ``<abs-tnotes> feedback <abs-path>``; a
    double-click with no file runs ``<abs-tnotes> feedback`` for a general report. The
    absolute ``tnotes`` is baked in (a Finder launch has no shell PATH — ADR-005).
    ...
    return (
        ...
        "        set cmd to " + tnotes_literal + " & \" feedback \" & quoted form of POSIX path of f\n"
        "        tell application \"Terminal\" to do script cmd\n"
        "    end repeat\n"
        "end open\n"
        "\n"
        "on run\n"
        "    tell application \"Terminal\" to do script " + tnotes_literal + " & \" feedback\"\n"
        "end run\n"
    )
...
def create_feedback_droplet(tnotes_path: str | None = None) -> bool:
    ...
    that does ``tell application "Terminal" to do script "<tnotes> feedback
    ...
    source = _applescript_source(tnotes_abs)
    desktop = Path.home() / "Desktop"
    droplet = desktop / "Send Feedback.app"
    try:
